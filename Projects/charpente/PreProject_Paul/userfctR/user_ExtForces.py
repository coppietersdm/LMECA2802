...
def user_ExtForces(PxF, RxF, VxF, OMxF, AxF, OMPxF, mbs_data, tsim, ixF):
    ...
    if ixF == mbs_data.extforce_id["ExtForce_0"]:
        P_Y1 = mbs_data.S1.P[2]
        P_Z1 = mbs_data.S1.P[3]
        V_Y1 = mbs_data.S1.V[2]
        V_Z1 = mbs_data.S1.V[3]
    ...
    if ixF == mbs_data.extforce_id["ExtForce_1"]:
        P_Y0 = mbs_data.S0.P[2]
        P_Z0 = mbs_data.S0.P[3]
        V_Y0 = mbs_data.S0.V[2]
        V_Z0 = mbs_data.S0.V[3]

        Fy = -((PxF[2]-P_Y0)*Kconn+Dconn*(VxF[2]-V_Y0))
        Fz = -((PxF[3]-P_Z0)*Kconn+Dconn*(VxF[3]-V_Z0))

    if ixF == mbs_data.extforce_id["ExtForce_2"]:
        P_Y3 = mbs_data.S3.P[2]
        P_Z3 = mbs_data.S3.P[3]
        V_Y3 = mbs_data.S3.V[2]
        V_Z3 = mbs_data.S3.V[3]
        ...
        mbs_data.WallVertF.append(FzWall)
        mbs_data.WallHorF.append(FyWall)

        Fy -= FyWall
        Fz -= FzWall

    if ixF == mbs_data.extforce_id["ExtForce_3"]:
        P_Y2 = mbs_data.S2.P[2]
        P_Z2 = mbs_data.S2.P[3]
        V_Y2 = mbs_data.S2.V[2]
        V_Z2 = mbs_data.S2.V[3]

        Fy = -((PxF[2]-P_Y2)*Kconn+Dconn*(VxF[2]-V_Y2))
        Fz = -((PxF[3]-P_Z2)*Kconn+Dconn*(VxF[3]-V_Z2))

    if ixF == mbs_data.extforce_id["ExtForce_4"]:
        P_Y5 = mbs_data.S5.P[2]
        P_Z5 = mbs_data.S5.P[3]
        V_Y5 = mbs_data.S5.V[2]
        V_Z5 = mbs_data.S5.V[3]

        Fy = -((PxF[2]-P_Y5)*Kconn+Dconn*(VxF[2]-V_Y5))
        Fz = -((PxF[3]-P_Z5)*Kconn+Dconn*(VxF[3]-V_Z5))
    
        #wall
        Fy -= ((PxF[2]-5)*Kconn+Dconn*VxF[2])
        Fz -= ((PxF[3])*Kconn+Dconn*VxF[3])


    if ixF == mbs_data.extforce_id["ExtForce_5"]:
        P_Y4 = mbs_data.S4.P[2]
        P_Z4 = mbs_data.S4.P[3]
        V_Y4 = mbs_data.S4.V[2]
        V_Z4 = mbs_data.S4.V[3]
